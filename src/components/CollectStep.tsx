...
export default function CollectStep({ onContinue }: Props) {
  ...
  // ─── Totals & continue ───────────────────────────────────────────────────
  const totalCount = driveSelected.size + localFiles.length + pastedFiles.length;
  ...
  function handleContinue() {
    ...
    onContinue([...fromDrive, ...fromLocal, ...fromPaste]);
  }
      ...
      >
        ...
        >
          ...
          <p className="text-sm font-semibold mt-2" style={{ fontFamily: "var(--font-inter)", color: pasteHint ? "#28347c" : "#666" }}>
            {pasteHint ? "Ahora pegá (Cmd+V / Ctrl+V)" : "Tocá acá y luego pegá"}
          </p>
